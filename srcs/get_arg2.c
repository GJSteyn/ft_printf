...
/*   Updated: 2018/08/17 10:43:11 by gsteyn           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_point(t_flags *flags, va_list ap)
{
	flags->arg = (unsigned long long)va_arg(ap, unsigned long long);
	if (flags->len == normal)
		flags->out = ft_itoa_base((unsigned int)flags->arg, 16, 0);
	else if (flags->len == hh)
		flags->out = ft_itoa_base((unsigned char)flags->arg, 16, 0);
	else if (flags->len == h)
		flags->out = ft_itoa_base((unsigned short)flags->arg, 16, 0);
	else if (flags->len == l)
		flags->out = ft_itoa_base((unsigned long)flags->arg, 16, 0);
	else if (flags->len == ll)
		flags->out = ft_itoa_base((unsigned long long)flags->arg, 16, 0);
	else if (flags->len == j)
		flags->out = ft_itoa_base((intmax_t)flags->arg, 16, 0);
	else if (flags->len == z)
		flags->out = ft_itoa_base((size_t)flags->arg, 16, 0);
}
